...
def device_io_control(
    file_handle, control_code, input_buffer, input_size, output_buffer, output_size
):
    ...
    if control_code == 0x4D014 and False:
        DeviceIoControl = windll.kernel32.DeviceIoControl
        bytes_returned = c_ulong()

        cDevice = int(file_handle)
        cIoControlCode = create_string_buffer(input_buffer)
        cIoControlCode_size = sizeof(cIoControlCode)
        cOutBuffer = create_string_buffer(bytes(output_buffer))
        cOutBuffer_size = sizeof(cOutBuffer)
        ...
        if not DeviceIoControl(
            ...
            print([i for i in cIoControlCode])
            print(bytes_returned)
            print("Error in DeviceIoControl:", windll.kernel32.GetLastError())
            raise IOError("DeviceIoControl failed")
        result = bytes_returned
    else:
        result = win32file.DeviceIoControl(
            file_handle, control_code, input_buffer, output_buffer, None
        )
...
if file_handle:
    try:
        ...
        device_io_control(
            file_handle, IOCTL_SCSI_GET_ADDRESS, None, 0, output_buffer, 8
        )

        # IOCTL_SCSI_PASS_THROUGH_DIRECT
        IOCTL_SCSI_PASS_THROUGH_DIRECT = 0x4D014
        input_buffer = b"\x2c\x00\x00\x00\x00\x00\x06\x00\x01\x00\x00\x00\x24\x00\x00\x00\x04\x00\x00\x00\x5e\x5f\xa9\x00\x00\x00\x00\x00\x12\x00\x00\x00\x24\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
